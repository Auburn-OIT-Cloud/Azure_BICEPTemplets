...
function RunMsiWithRetry {
    ...
    do {
        ...
        if ($retryCount -gt 0) {
            Get-Log  "Retrying $modeAndDisplayName in $retryTimeToSleepInSec seconds because it failed with Exit code=$sts This will be retry number $retryCount"
            Start-Sleep -Seconds $retryTimeToSleepInSec
        }

        Get-Log ( "$modeAndDisplayName" + $(if ($msiLogVerboseOutput) { " with verbose msi logging" } else { "" }))
    ...
    if ($sts -eq 1618) {
        Get-Log  "Stopping retries for $modeAndDisplayName. The last attempt failed with Exit code=$sts which is ERROR_INSTALL_ALREADY_RUNNING"
        throw "Stopping because $modeAndDisplayName finished with Exit code=$sts"
    }
    else {
        Get-Log "$modeAndDisplayName finished with Exit code=$sts"
    }
...
function InstallRDAgents {
    ...
    Get-Log  "Boot loader folder is $AgentBootServiceInstallerFolder"
    $AgentBootServiceInstaller = $AgentBootServiceInstallerFolder + '\WVD-BootLoader.msi'

    Get-Log  "Agent folder is $AgentInstallerFolder"
    ...
    Get-Log  "Installing RD Infra Agent on VM $AgentInstaller"
    ...
    Get-Log  "Installing RDAgent BootLoader on VM $AgentBootServiceInstaller"
    ...
    Get-Log  "Starting service $bootloaderServiceName"
...
Get-Log "Calling functions to install agent and bootloader"
...
Get-log "Wait for IsRegistered status"
$isRegistered = 0
DO {
